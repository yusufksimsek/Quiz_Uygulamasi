...
import 'package:quiz_uygulamasi/QuizUygulamasi/VeritabaniYardimcisi.dart';
...
class Bayraklardao{
  ...
  Future<List<Bayraklar>> rastgele5Getir() async {
    ...
    List<Map<String,dynamic>> maps = await db.rawQuery("SELECT * FROM bayraklar ORDER BY "
        "RANDOM() LIMIT 5");
  ...
  Future<List<Bayraklar>> rastgele3YanlisGetir(int bayrak_id) async {
    ...
    List<Map<String,dynamic>> maps = await db.rawQuery("SELECT * FROM bayraklar "
        "WHERE bayrak_id != $bayrak_id ORDER BY RANDOM() LIMIT 3");
